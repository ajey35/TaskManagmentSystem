...
const Onprocess = () => {
  ...
  const todos = tasks.filter((task) => task.status === "In Progress");
  const len = todos.length;

  return (
    <>
    <div className=" grid mx-auto bg-gray-100 rounded-lg shadow-lg">
    <h1 className=" text-3xl font-bold underline mb-1 flex justify-center"> ON PROCESS</h1>
      <div className="grid grid-cols-2 gap-4 p-4 ">
    {todos.map((todo) => (
      <div key={todo.id}
       className="flex flex-col bg-white p-4 rounded-lg shadow-md m-5"
